...
module.exports.getSinhVienThucHienList = function (gv_ma ,callback) {
    pool.query('SELECT * FROM sinhvien_dk_detai as svdkdt , sinhvien as sv , detai as dt WHERE svdkdt.sv_ma = sv.sv_ma and svdkdt.ttdk_ma=\'yc\' and dt.gv_ma=$1', [gv_ma], (error, results) => {
        callback(error, results.rows);
    });
};
...
module.exports.updateSinhVienDangKiAccept = function (dt_ma, sv_ma, callback) {
    pool.query('UPDATE sinhvien_dk_detai set ttdk_ma=\'yc\' WHERE dt_ma=$1 and sv_ma=$2 ', [dt_ma,sv_ma], (error, result) => {
        callback(error, result);
    });
};
